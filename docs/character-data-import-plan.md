...
> Plan only — no character files have been generated by this importer yet. Scope: read `genshin-db/` and `paimon-moe/` (read-only), write only inside `teyvat-personality/`.

## Goal

For every entry in `src/data/characters/characters.json` (125 entries), generate a file `src/data/characters/{id}.json` following the exact schema of the existing `nahida.json`:
...
`characters.json` ids use snake_case (`hu_tao`, `arataki_itto`). `genshin-db/src/data/English/characters/*.json` filenames use lowercase with no separators (`hutao.json`, `aratakiitto.json`). To find the source file for a given id:
...
| ----------------------- | ----------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| `id`, `name`            | `characters.json` (already correct)                                                                                           | —                       |
...
- `traveler_anemo`, `traveler_dendro`, `traveler_electro`, `traveler_geo`, `traveler_hydro`, `traveler_pyro`, `traveler_cryo` (7 ids) — genshin-db has no per-element Traveler entry, only `aether.json` (Male) / `lumine.json` (Female), both with empty `title` and empty `birthday`/`birthdaymmdd` (no canon birthday). `paimon-moe/src/data/birthdays.js` has no traveler entry either.
  - Use the shared Traveler values for all seven files: `description`, `weapon`, and `rarity` come from the corresponding Paimon Traveler data where present; those values are identical across the available variants, so `traveler_cryo` reuses the same shared values. `title` and `birthday` are `null`. `traveler_cryo` changes only `element` to `"Cryo"`.
  ...
  - **Resolved — `gender`:** Traveler is a twin character (Aether = Male, Lumine = Female), not a single value. `gender: null` is intentional on all 7 `traveler_*` files and is excluded from the missing-data report.

**4 genshin-db files have no matching characters.json id** (not relevant to this task, no action): `aether`, `lumine`, `manekin`, `manekina`.
...
Practical effect: running the import script again after this task (e.g. when a new character patch drops) only (a) creates files for ids that are new to `characters.json`, and (b) refreshes factual fields on existing files when upstream provides a concrete value. Existing known factual data and any personality research a human has added survive every re-run.
...
1. Build the id → genshin-db-file map (as above); for the 9 gap ids, fall back to paimon-moe sources where available.
...
4. For the 7 `traveler_*` ids, use the shared Traveler profile and derive the element from the id; `traveler_cryo` differs only by using `"Cryo"` as its element. Derive `region` from that element via `src/data/masters/regions.json` (element → region name). Keep `gender: null` for the documented twin duality and do not flag it as a gap.
5. For any other field that can't be sourced from either repo, write `null` (never fabricate) and add a line to the gap report.
6. Write a single summary file, e.g. `src/data/characters/_missing-data-report.md` (or `.json`), listing per-id which fields were written as `null` and why (no source / source conflict). Traveler `gender` and alyosha/odette `birthday` are excluded from this report per the resolutions above.
