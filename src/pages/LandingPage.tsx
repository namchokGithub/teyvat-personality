...
import { beginQuizFromNavigation, useDialogAccessibility, hasSavedQuizProgress } from "../hooks";
import { t } from "../i18n";
...
export function LandingPage({ locale }: { locale: Locale }) {
  ...
  const [showNameDialog, setShowNameDialog] = useState(() => Boolean((location.state as { requestName?: boolean } | null)?.requestName));
  ...
  const confirmName = () => { savePlayerName(name); beginQuizFromNavigation(); closeNameDialog(); navigate("/quiz", { state: { beginQuiz: true } }); };
