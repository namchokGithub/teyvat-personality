...
export function CharacterPage({ locale }: { locale: Locale }) {
  ...
  return <main className="character-page"><PageContainer className="character-shell"><Link className="back-link" to="/result"><ArrowLeft size={17} />{t(locale, "back")}</Link><article className="character-profile"><div className="character-profile__art" aria-hidden="true"><Sparkles size={42} /><span>{initial}</span></div><div className="character-profile__content"><span className="eyebrow">{show(character.region, t(locale, "unavailable"))} · {show(character.element, t(locale, "unavailable"))}</span><h1>{character.name}</h1><p className="result-title">{title}</p><div className="character-facts"><span>{show(character.weapon, t(locale, "unavailable"))}</span><span>{character.rarity ? Array.from({ length: character.rarity }, (_, index) => <Star key={index} size={14} fill="currentColor" />) : t(locale, "unavailable")}</span></div><h2>{t(locale, "characterAbout")}</h2><p>{description}</p><div className="data-notice">{t(locale, "factualNotice")}</div></div></article></PageContainer></main>;
}
