...
const question = (id: string, dimensionId: DimensionId, th: string, en: string, answers: ScoredQuizQuestion["answers"]): ScoredQuizQuestion => ({ id, dimensionId, prompt: { th, en }, answers });

export const questions: ScoredQuizQuestion[] = [
  question("q_social_01", "social", "คุณมาถึงงานที่แทบไม่รู้จักใคร คุณจะเริ่มอย่างไร?", "You arrive at a gathering where you barely know anyone. How do you begin?", [
  ...
  question("q_social_02", "social", "ในทีมใหม่ที่ยังไม่คุ้นกัน คุณมักมีบทบาทแบบไหน?", "On a new team that does not know each other yet, what role do you take?", [
  ...
  question("q_social_03", "social", "เมื่อเพื่อนชวนออกไปข้างนอกแบบกะทันหัน คุณมักจะ…", "When a friend invites you out at the last minute, you usually…", [
  ...
  question("q_social_04", "social", "เมื่อมีคนใหม่เข้ามาในกลุ่มเดิมของคุณ คุณจะ…", "When someone new joins your established group, you…", [
  ...
  question("q_social_05", "social", "ในงานเลี้ยงรวมญาติที่หลายคนคุณไม่เคยเจอมาก่อน คุณจะทำอย่างไร?", "At a family gathering full of relatives you have never met, what do you do?", [
    answer("greet_first", "เดินไปทักทายและแนะนำตัวก่อน", "Walk over and introduce yourself first.", { social: 3, expression: 1 }, { confidence: 0.3, enthusiasm: 0.2, passion: 0.1 }),
    answer("stay_near_family", "อยู่ใกล้พ่อแม่หรือญาติที่รู้จักไว้ก่อน", "Stay near a parent or relative you already know.", { social: 1, responsibility: 1 }, { loyalty: 0.2 }),
    answer("observe_relatives", "สังเกตว่าใครเป็นใครก่อนตัดสินใจเข้าไปคุย", "Observe who's who before deciding whom to approach.", { social: -1, decision: 2 }, { introspection: 0.2 }),
    answer("wait_quietly", "นั่งเงียบ ๆ รอให้มีคนมาชวนคุยเอง", "Sit quietly and wait for someone to approach you.", { social: -3, expression: -1 }, { independence: 0.2 }),
  ]),
  question("q_social_06", "social", "เพื่อนร่วมงานชวนไปกินข้าวเที่ยงกะทันหันในวันที่คุณวางแผนไว้แล้ว คุณจะ…", "A coworker invites you to lunch out of nowhere on a day you already had plans. You…", [
    answer("accept_lunch", "ตอบรับทันทีเพราะอยากรู้จักเขามากขึ้น", "Say yes right away because you want to get to know them better.", { social: 3, adventure: 1 }, { enthusiasm: 0.2 }),
    answer("invite_more_coworkers", "ชวนเพื่อนคนอื่นไปด้วยให้คึกคักขึ้น", "Invite more coworkers along to make it livelier.", { social: 3, expression: 1 }, { leadership: 0.2, humor: 0.2 }),
    answer("check_schedule", "เช็คตารางเดิมก่อนว่าพอปรับได้ไหม", "Check your existing schedule first to see if it can flex.", { social: -1, lifestyle: 2 }, { stability: 0.2 }),
    answer("eat_alone_as_planned", "ขอตัวไปทานคนเดียวตามแผนเดิม", "Excuse yourself and eat alone as originally planned.", { social: -3, expression: -1 }, { introspection: 0.3 }),
  ]),
  question("q_decision_01", "decision", "ข้อมูลสำคัญสองแหล่งให้คำตอบไม่ตรงกัน คุณจะ…", "Two important sources give conflicting answers. You…", [
  ...
  question("q_decision_02", "decision", "เมื่อเพื่อนขอคำแนะนำเรื่องยาก คุณจะเริ่มจาก…", "When a friend asks for advice on something difficult, you start by…", [
  ...
  question("q_decision_03", "decision", "คุณต้องตัดสินใจภายใต้เวลาจำกัด คุณจะ…", "You must decide with limited time. You…", [
  ...
  question("q_decision_04", "decision", "กฎข้อหนึ่งทำให้เกิดผลลัพธ์ไม่ยุติธรรม คุณจะ…", "A rule produces an unfair outcome. You…", [
  ...
  question("q_decision_05", "decision", "เพื่อนสองคนเล่าเหตุการณ์เดียวกันแต่รายละเอียดขัดกัน คุณจะ…", "Two friends describe the same event but the details contradict. You…", [
    answer("confirm_with_evidence", "หาหลักฐานหรือคนที่สามมายืนยันก่อนเชื่อ", "Look for evidence or a third party to confirm before believing either.", { decision: 3, lifestyle: 1 }, { knowledge: 0.3, curiosity: 0.2 }),
    answer("ask_informed_person", "ถามคนที่รู้เรื่องมากกว่าให้ช่วยตัดสิน", "Ask someone more informed to help judge.", { decision: 2, social: 1 }, { learning: 0.2 }),
    answer("believe_aligned_side", "เลือกเชื่อฝั่งที่ตรงกับสิ่งที่คุณให้ความสำคัญ", "Believe the side that aligns with what you value.", { decision: -2, responsibility: 1 }, { ideals: 0.3, identity: 0.2 }),
    answer("trust_first_instinct", "เชื่อตามความรู้สึกแรกที่มีต่อเรื่องนี้", "Go with your first instinct about the situation.", { decision: -3, adventure: 1 }, { confidence: 0.2 }),
  ]),
  question("q_decision_06", "decision", "ระบบล่มกลางงานสำคัญและคุณมีเวลาแก้ปัญหาไม่มาก คุณจะ…", "A system fails in the middle of something important and you have little time to fix it. You…", [
    answer("follow_checklist", "ใช้ขั้นตอนที่เคยเตรียมไว้แก้ปัญหาตามลำดับ", "Follow a prepared checklist to fix it step by step.", { decision: 3, lifestyle: 2 }, { discipline: 0.2 }),
    answer("gather_critical_info", "รีบรวบรวมข้อมูลสำคัญที่สุดให้เร็วที่สุด", "Quickly gather the most critical information first.", { decision: 2, adventure: 1 }, { knowledge: 0.2 }),
    answer("minimize_impact", "เลือกวิธีที่กระทบคนอื่นน้อยที่สุดก่อน", "Choose the fix that affects other people the least.", { decision: -2, responsibility: 2 }, { empathy: 0.3, selflessness: 0.2 }),
    answer("act_immediately", "ลองสิ่งที่รู้สึกว่าน่าจะได้ผลทันที", "Try whatever feels like it will work and act immediately.", { decision: -3, adventure: 2 }, { confidence: 0.2, passion: 0.2 }),
  ]),
  question("q_lifestyle_01", "lifestyle", "เมื่อมีทริปที่ต้องเตรียมหลายอย่าง คุณจะ…", "When preparing for a trip with many moving parts, you…", [
  ...
  question("q_lifestyle_02", "lifestyle", "พื้นที่ทำงานของคุณมักเป็นแบบไหน?", "What is your workspace usually like?", [
  ...
  question("q_lifestyle_03", "lifestyle", "ถ้าแผนทั้งวันถูกเปลี่ยนกะทันหัน คุณจะ…", "If your whole day is suddenly rearranged, you…", [
  ...
  question("q_lifestyle_04", "lifestyle", "งานใหญ่ที่มีเวลาหลายสัปดาห์ คุณมัก…", "For a large task with several weeks to complete it, you usually…", [
  ...
  question("q_lifestyle_05", "lifestyle", "ก่อนเริ่มโปรเจกต์ใหญ่ที่ต้องประสานหลายฝ่าย คุณจะ…", "Before starting a big project that needs coordinating with many people, you…", [
    answer("plan_timeline", "วางไทม์ไลน์และรายการสิ่งที่ต้องทำไว้ล่วงหน้า", "Lay out a timeline and to-do list in advance.", { lifestyle: 3 }, { discipline: 0.3, reliability: 0.2, resolve: 0.1 }),
    answer("prepare_and_adjust", "เตรียมสิ่งจำเป็นก่อนแล้วปรับไปตามหน้างาน", "Prepare the essentials first and adjust as it unfolds.", { lifestyle: 1, adventure: 1 }, { adaptability: 0.2 }),
    answer("wait_for_team_direction", "รอดูทิศทางของทีมก่อนค่อยวางรายละเอียด", "Wait to see the team's direction before planning details.", { lifestyle: -2 }, { freedom: 0.2 }),
    answer("follow_project_pace", "ปล่อยให้จังหวะงานพาไปตามสถานการณ์", "Let the project's pace carry you along as it goes.", { lifestyle: -3, adventure: 1 }, { freedom: 0.2 }),
  ]),
  question("q_lifestyle_06", "lifestyle", "ตารางทั้งสัปดาห์ของคุณถูกยกเลิกกะทันหัน คุณจะ…", "Your whole week's schedule gets cancelled without warning. You…", [
    answer("rebuild_schedule", "จัดตารางใหม่ทันทีเพื่อไม่ให้เวลาว่างเปล่า", "Rebuild a new schedule right away so time isn't wasted.", { lifestyle: 3, decision: 1 }, { discipline: 0.2, adaptability: 0.2 }),
    answer("keep_key_commitment", "รักษานัดสำคัญที่สุดไว้ ส่วนที่เหลือค่อยว่ากัน", "Keep the most important commitment and figure out the rest later.", { lifestyle: 1, responsibility: 1 }, { stability: 0.2 }),
    answer("seize_free_time", "ถือเป็นโอกาสทำสิ่งที่ไม่เคยมีเวลาทำ", "Treat it as a chance to do things you never had time for.", { lifestyle: -2, adventure: 2 }, { freedom: 0.2, curiosity: 0.2, optimism: 0.2 }),
    answer("let_time_pass", "ปล่อยเวลาผ่านไปโดยไม่วางแผนอะไรใหม่", "Let the time pass without planning anything new.", { lifestyle: -3 }, { adaptability: 0.2 }),
  ]),
  question("q_adventure_01", "adventure", "คุณพบเส้นทางใหม่ที่อาจพาไปถึงที่หมายเร็วกว่า คุณจะ…", "You find a new route that might reach the destination faster. You…", [
  ...
  question("q_adventure_02", "adventure", "มีโอกาสเรียนทักษะใหม่ที่ยากและอาจล้มเหลว คุณจะ…", "You have a chance to learn a difficult skill and might fail. You…", [
  ...
  question("q_adventure_03", "adventure", "เมื่อเกิดปัญหาที่ไม่มีคู่มือบอกวิธีแก้ คุณจะ…", "When a problem has no manual solution, you…", [
  ...
  question("q_adventure_04", "adventure", "คุณได้รับคำชวนไปยังสถานที่ไม่คุ้นเคย คุณจะ…", "You are invited somewhere unfamiliar. You…", [
  ...
  question("q_adventure_05", "adventure", "มีทางลัดที่ไม่เคยลองแต่อาจประหยัดเวลาได้มาก คุณจะ…", "There's a shortcut you've never tried that could save a lot of time. You…", [
    answer("try_shortcut", "ลองไปเลยพร้อมรับความเสี่ยงที่อาจเกิดขึ้น", "Try it right away, accepting whatever risk comes with it.", { adventure: 3, decision: 1 }, { curiosity: 0.2, confidence: 0.2 }),
    answer("research_shortcut", "หาข้อมูลทางลัดนี้ก่อนตัดสินใจ", "Look into the shortcut before deciding.", { adventure: 1, decision: 2 }, { knowledge: 0.2 }),
    answer("ask_group_risk", "ถามคนที่ไปด้วยว่าโอเคกับความเสี่ยงนี้ไหม", "Ask the people with you if they're okay with the risk.", { adventure: -1, responsibility: 2 }, { empathy: 0.2 }),
    answer("stay_trusted_route", "เลือกทางเดิมที่มั่นใจกว่า", "Stick with the route you already trust.", { adventure: -3, lifestyle: 1 }, { stability: 0.2 }),
  ]),
  question("q_adventure_06", "adventure", "มีคนชวนคุณไปลองกิจกรรมที่ไม่เคยทำมาก่อนในที่ที่ไม่คุ้นเคย คุณจะ…", "Someone invites you to try a new activity in an unfamiliar place. You…", [
    answer("say_yes_curious", "ตอบตกลงเพราะอยากรู้ว่าจะเป็นอย่างไร", "Say yes because you want to find out what it's like.", { adventure: 3, social: 1 }, { curiosity: 0.3 }),
    answer("check_details_first", "ไปถ้าตรวจสอบรายละเอียดและความปลอดภัยได้ก่อน", "Go if you can check the details and safety beforehand.", { adventure: 1, decision: 2 }, { responsibility: 0.2 }),
    answer("go_if_familiar_person", "ไปถ้ามีคนที่รู้จักไปด้วยกัน", "Go if someone you know is coming too.", { adventure: -1, social: 2 }, { loyalty: 0.2 }),
    answer("decline_unfamiliar", "ปฏิเสธและเลือกทำสิ่งที่คุ้นเคยกว่า", "Decline and choose something more familiar.", { adventure: -3 }, { stability: 0.2 }),
  ]),
  question("q_responsibility_01", "responsibility", "ในงานกลุ่มมีส่วนสำคัญที่ยังไม่มีใครรับ คุณจะ…", "An important part of a group task has no owner. You…", [
  ...
  question("q_responsibility_02", "responsibility", "คุณให้สัญญาไว้ แต่เกิดเหตุที่ทำให้ทำตามได้ยาก คุณจะ…", "You made a promise, but an event makes it difficult to keep. You…", [
  ...
  question("q_responsibility_03", "responsibility", "คุณเห็นคนหนึ่งทำพลาดและกำลังถูกตำหนิ คุณจะ…", "You see someone make a mistake and receive blame. You…", [
  ...
  question("q_responsibility_04", "responsibility", "คุณมีหน้าที่ประจำที่ไม่มีใครเห็นผลทันที คุณจะ…", "You have a recurring duty whose results are not immediately visible. You…", [
  ...
  question("q_responsibility_05", "responsibility", "ในกิจกรรมของชมรม มีขั้นตอนสำคัญที่ยังไม่มีใครอาสาทำ คุณจะ…", "In a club activity, an important step still has no volunteer. You…", [
    answer("volunteer_self", "อาสาทำเองเพื่อให้กิจกรรมเดินหน้าต่อได้", "Volunteer yourself so the activity keeps moving.", { responsibility: 3 }, { reliability: 0.3, leadership: 0.2, resolve: 0.2 }),
    answer("find_right_person", "ช่วยหาคนที่เหมาะสมและติดตามให้ทำจริง", "Help find the right person and follow up to make sure it happens.", { responsibility: 2, social: 1 }, { leadership: 0.2 }),
    answer("finish_own_part", "ทำเฉพาะส่วนที่ตัวเองรับผิดชอบให้เสร็จก่อน", "Finish only your own assigned part first.", { responsibility: -1, lifestyle: 1 }, { independence: 0.2 }),
    answer("wait_for_handler", "รอดูว่าใครจะเป็นคนจัดการเรื่องนี้", "Wait to see who ends up handling it.", { responsibility: -3 }, { freedom: 0.1 }),
  ]),
  question("q_responsibility_06", "responsibility", "คุณเห็นเพื่อนร่วมทีมทำผิดพลาดและกำลังโดนต่อว่าต่อหน้าคนอื่น คุณจะ…", "You see a teammate make a mistake and get called out in front of others. You…", [
    answer("share_blame_and_fix", "เข้าไปช่วยแก้ไขและรับผิดชอบร่วมด้วย", "Step in to help fix it and share the responsibility.", { responsibility: 3, social: 1 }, { empathy: 0.2, reliability: 0.2 }),
    answer("clarify_facts", "ช่วยอธิบายข้อเท็จจริงให้เกิดความเข้าใจที่ถูกต้อง", "Help clarify the facts so the situation is understood fairly.", { responsibility: 2, decision: 2 }, { ideals: 0.2 }),
    answer("support_privately", "ให้กำลังใจเขาทีหลังแบบส่วนตัว", "Offer encouragement privately afterward.", { responsibility: 1, expression: -1 }, { loyalty: 0.2 }),
    answer("stay_out_of_it", "ไม่เข้าไปยุ่งเพราะไม่ใช่เรื่องของตัวเอง", "Stay out of it since it's not your situation.", { responsibility: -3, social: -1 }, { independence: 0.1 }),
  ]),
  question("q_expression_01", "expression", "เมื่อคุณภูมิใจในผลงานชิ้นหนึ่ง คุณมักจะ…", "When you are proud of something you made, you usually…", [
  ...
  question("q_expression_02", "expression", "เมื่อไม่เห็นด้วยกับคนที่มีอำนาจกว่า คุณจะ…", "When you disagree with someone more powerful, you…", [
  ...
  question("q_expression_03", "expression", "เมื่อรู้สึกหนักใจกับเรื่องส่วนตัว คุณมักจะ…", "When something personal weighs on you, you usually…", [
  ...
  question("q_expression_04", "expression", "เมื่อมีเรื่องตลกเกิดขึ้นในกลุ่ม คุณมักจะ…", "When something funny happens in a group, you usually…", [
  ...
  question("q_expression_05", "expression", "เมื่อคุณทำสิ่งที่ยากสำเร็จเป็นครั้งแรก คุณมักจะ…", "When you accomplish something difficult for the first time, you usually…", [
    answer("tell_others_excitedly", "รีบบอกคนใกล้ตัวด้วยความตื่นเต้น", "Excitedly tell the people close to you right away.", { expression: 3, social: 2 }, { selfExpression: 0.3, confidence: 0.2, enthusiasm: 0.2, optimism: 0.1 }),
    answer("tell_trusted_person", "เล่าให้คนที่ไว้ใจฟังและถามความเห็น", "Tell someone you trust and ask what they think.", { expression: 1, social: 1 }, { sensitivity: 0.2 }),
    answer("let_result_speak", "ปล่อยให้ผลลัพธ์พูดแทนตัวเอง", "Let the result speak for itself.", { expression: -1, decision: 1 }, { independence: 0.2 }),
    answer("keep_feeling_private", "เก็บความรู้สึกนี้ไว้คนเดียวก่อน", "Keep the feeling to yourself for now.", { expression: -3, social: -1 }, { introspection: 0.2 }),
  ]),
  question("q_expression_06", "expression", "เพื่อนในกลุ่มเผลอพูดอะไรตลก ๆ ขึ้นมา คุณมักจะ…", "A friend in the group accidentally says something funny. You usually…", [
    answer("riff_on_joke", "ต่อมุกให้สนุกขึ้นไปอีก", "Riff on it to make it even funnier.", { expression: 3, social: 2 }, { humor: 0.3, enthusiasm: 0.2 }),
    answer("laugh_and_join", "หัวเราะและร่วมสนุกไปกับกลุ่ม", "Laugh and join in with the group naturally.", { expression: 1, social: 2 }, { acceptance: 0.2 }),
    answer("smile_without_adding", "ยิ้มแต่ไม่ได้พูดอะไรเพิ่ม", "Smile but don't add anything.", { expression: -1, social: -1 }, { sensitivity: 0.1 }),
    answer("stay_composed_listening", "วางเฉยและฟังบทสนทนาต่อไป", "Stay composed and keep listening.", { expression: -3, decision: 1 }, { stability: 0.2 }),
  ]),
];
